...
transcriptsRouter.get("/by-recording/:recordingId", async (req, res) => {
...
transcriptsRouter.patch("/:id/segments/:segmentId", async (req, res) => {
  const userId = req.user!.id;
...
transcriptsRouter.delete("/:id/segments/:segmentId", async (req, res) => {
  await TranscriptSegments.deleteSegment(req.user!.id, req.params.segmentId);
  res.status(204).end();
});

transcriptsRouter.post("/:id/segments/reorder", async (req, res) => {
...
transcriptsRouter.post("/:id/segments/merge", async (req, res) => {
...
transcriptsRouter.patch("/:id/speakers/:speakerLabel", async (req, res) => {
  const userId = req.user!.id;
...
transcriptsRouter.get("/:id/markdown", async (req, res) => {
  const userId = req.user!.id;
  const transcript = await Transcripts.findForUser(userId, req.params.id);
  if (!transcript) {
    res.status(404).json({ error: "Transcript not found" });
...
transcriptsRouter.post("/:id/save-to-drive", async (req, res) => {
  const userId = req.user!.id;
  const transcript = await Transcripts.findForUser(userId, req.params.id);
  if (!transcript) {
    res.status(404).json({ error: "Transcript not found" });
